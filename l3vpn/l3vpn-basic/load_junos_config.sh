...
main() {
    if [ -z "$1" ]; then
        ...
        for i in vix1 vce2-1 vce2-2 vce2-3 vce2-4 vce2-5 ; do sshpass -padmin@123 scp -o "StrictHostKeyChecking=no" $i-$state.conf admin@clab-ml-$1-$i:/var/tmp/;done
        for i in vix1 vce2-1 vce2-2 vce2-3 vce2-4 vce2-5 ; do sshpass -padmin@123 ssh admin@clab-ml-$1-$i "edit;load merge /var/tmp/$i-$state.conf;commit and-quit";done
    fi
}
get_help() {
    echo "Usage: $0 [multi-lab number]"
