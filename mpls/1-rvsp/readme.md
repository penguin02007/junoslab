...
5) Configure LSPs I and K and LSPs J and L so that they use two distinct physical paths to the egress node. The paths should take three hops each. You may not use administrative groups in this step.
6) Configure LSPs M and O and LSPs N and P so that they use two distinct physical paths to the egress node. LSPs M and O should use only the “green” links, and LSPs N and P should use only the “red” links.
7) Configure all LSPs except A, B, S, and T to reserve 60 Mbps of bandwidth.
...
9) Configure LSPs A, B, E, F, I, J, Q, R, S, and T to ensure that they have higher priority for bandwidth reservation than the remaining LSPs.
10) Make sure that if LSPs K, L, O, and P must be preempted, the ingress router will attempt to re-signal the LSP before tearing it down.
11) Configure automatic optimization for the LSPs I, J, K, L, M, N, O, and P. Set the optimize timer to 8 hours. Make sure that the ingress routers attempt to re-signal the LSP before tearing it down.
...
15) Configure per-flow load balancing over LSPs N and P. Similarly, configure per-flow load balancing over LSPs M and O.
